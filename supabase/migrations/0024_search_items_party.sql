-- Extend search_items with supplier-aware ranking + per-supplier
-- last-cost pre-fill for the Receive screen.
--
-- Two changes:
--   1. Add p_party_id parameter. When set together with p_screen='receive',
--      items with a prior receive line from this party rank above items
--      without one. Activated favorites still beat catalog candidates.
--   2. Add `last_cost numeric` to the returned columns. When p_party_id
--      is set and screen='receive', filled with the unit_amount from the
--      most recent posted receive line of (this shop, this party, this
--      item). Null otherwise — the Sale screen doesn't read it.
--
-- The supplier-specific last cost gives the Receive screen a defensible
-- "pre-fill cost so the cashier confirms instead of types" default, which
-- mirrors the price-seeding we ship on Sale (decisions.md Q14). Per-line
-- lookup avoids a denormalized column (`item_party_last_cost`) we'd then
-- have to keep in sync on every receive/void.
--
-- Drops the previous 5-arg signature so there's only one canonical name.
...
declare
  ...
  -- Only use party for ranking + cost lookup on the Receive screen.
  -- A party_id passed with screen != 'receive' is silently ignored, so
  -- callers don't have to branch by screen on the client side.
...
begin
  ...
  with activated as (
    select
      ...
      i.sale_price,
      -- Supplier-specific last cost: most recent unit_amount from a
      -- posted receive line where (this shop, this party, this item).
      -- unit_amount is the entered per-unit cost the cashier typed
      -- (post_receive populates it directly from p_lines.unit_cost).
      case
      ...
      -- Supplier-history boost: items received from this party in the
      -- past sort above other activated items. Only meaningful when
      -- v_party_id is set; null collapses to 0.
      case
