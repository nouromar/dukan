...
-- ---------------------------------------------------------------------------
-- get_shop_item_stocks — fast batched stock lookup.
-- ---------------------------------------------------------------------------
--
-- Used by the Sale screen's negative-stock probe right after a post:
-- one round trip returning just (current_stock, base unit, display name)
-- per shop_item — much lighter than calling get_shop_item N times
-- (which aggregates units + aliases + barcodes per item).

create or replace function public.get_shop_item_stocks(
  p_shop_id uuid,
...
begin
  if not public.auth_can_access_shop(p_shop_id) then
    raise exception 'Not allowed to read shop stocks';
  end if;

  return query
  select
    si.id as shop_item_id,
    public.shop_item_display_name(si.id, p_locale) as display_name,
    si.base_unit_code,
    public.tr(u.default_label, u.label_translations, p_locale) as base_unit_label,
    si.current_stock,
    si.reorder_threshold
  from public.shop_item si
  join public.unit u on u.code = si.base_unit_code
  where si.shop_id = p_shop_id
    and si.id = any(p_shop_item_ids);
end;
$$;

revoke all on function public.get_shop_item_stocks(uuid, uuid[], text) from public;
grant execute on function public.get_shop_item_stocks(uuid, uuid[], text) to authenticated;
