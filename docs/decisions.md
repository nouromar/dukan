...
| 14 | Staff onboarding flow | **Auto-claim on sign-in.** Owner enters phone OR email + role in Setup → `create_shop_invite` writes a pending row → cashier signs in normally on mobile or portal → `claim_pending_invites_for_me()` runs in the session-load hook and creates the membership. No SMS infra, no deep links, no accept step. Owner communicates the login contact to the cashier via WhatsApp / in person (which they already do). Full design: [`docs/staff-onboarding.md`](staff-onboarding.md). (DECIDED 2026-06-15 by @nouromar — chose this over the original SMS-deep-link plan in `shop-admin-portal-alignment.md` § 8.3 to ship faster and avoid the SMS provider dependency.) | DECIDED |
