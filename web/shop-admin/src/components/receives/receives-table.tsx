// Receives history table. Reads list_receives output + renders columns.
// Voided pairs collapse to one row via the RPC LEFT JOIN, same as
// SalesTable. Owing (paid_amount < total_amount) renders a Credit
// chip mirroring the sales convention.
...
export function ReceivesTable({
  ...
  const t = useTranslations("receives");
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        columns={columns}
        data={filtered}
        onRowClick={(row) => router.push(`/receives/${row.txn_id}`)}
        empty={
          <EmptyState
            title={t("empty.title")}
